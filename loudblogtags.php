...
function link_prev($content) {
    ...
    if (($here > 1) AND (!isset($_GET['id']))) {
      if (isset($att['impact'])) {
        $return = '<a href="impact2.php'.addToUrl('page', ($here - 1)).'" title="previous page">';
      } else {
...
function link_next ($content) {
    ...
    if (!isset($_GET['id']) AND ($nextpage == true)) {
      if (isset($att['impact'])) {
        $return  = '<a href="impact2.php'.addToUrl('page', ($here + 1)).'" title="next page">';
      } else {
...
/**
 * bill: generates only text of permalink for the posting (works within postings-loop)
 *
 * @param unknown_type $content
 * @return unknown
 */
function link_permalink_text ($content) {
    global $settings, $currentid;

    if ((!isset($_GET['id'])) OR ($_GET['id'] != $currentid)) {
      $return = "index.php?id=" .$currentid;
    } else {
      $return = "";
    }
    return $return;
...
function permalink_to_post ($content) {
    global $settings, $currentid;
    return $settings['url']."/index.php?id={$currentid}";
}
...
function message_input() {
    global $postings, $currentid;
    return $postings[$currentid]['message_input'];
}
...
//redirect or direct link?
if ($settings['countweb'] == "1") {             # what does this mean
    $before = "get.php?web=";
} else {
    $before = "audio/";
}

if (!veryempty($postings[$currentid]['video_file'])) {
    if ($postings[$currentid]['filelocal'] == 1) {           # what does filelocal mean??
        $audio = $before . $postings[$currentid]['video_file'];
    } else {
        $audio = $postings[$currentid]['video_file'];
        if ($settings['countweb'] == "1") {
            $audio = $before.$audio;
        }
    }
...
/**
 * returns the path to the audio file (works within postings-loop)
 *
...
global $settings;
$before = "audio/";
return $settings['url'].$before.$postings[$currentid]['audio_file'];
}
...
global $postings;
global $currentid;
$before = "https://www.youtube.com/embed/";
$yt_link= $postings[$currentid]['video_link'];
$video_id = strrchr($yt_link, '=');
$video_id= substr($video_id,1); 
return $before.$video_id;
}
// </priyanka>
#################################################
#################################################
function imagefile() {
//returns the filename of the image-file (works within postings-loop)
global $postings;
global $currentid;
global $settings;
$before = "audio/";
return $settings['url'].$before.$postings[$currentid]['image_file'];
}
#################################################
#################################################
...
if (isset($_POST['commentsubmit'])) {
    ...
    $commentingokay = true;
    // disable comments for now - just clogging up database (bill)
...
if ($allcomm == "false") {
    ...
    $dosql = "SELECT * FROM ".$GLOBALS['prefix']."lb_comments
              WHERE posting_id = ".$currentid." ORDER BY posted DESC;";
...
function if_videolink($content){
global $postings;
global $currentid;

if (!veryempty($postings[$currentid]['video_link'])) {
    return trim(fullparse (stripcontainer($content)));
} else { return ""; }
...
function if_photo ($content) {
global $postings;
global $currentid;

if (!veryempty($postings[$currentid]['image_file']) &&
    !veryempty($postings[$currentid]['image_on']) &&
    isset($postings[$currentid]['image_on'])) {
   return trim(fullparse (stripcontainer($content)));
} else { return ""; }
}
...
if (!veryempty($postings[$currentid]['image_file']) &&
    ...
    isset($postings[$currentid]['image_on'])) {
    return "";
} else { return trim(fullparse (stripcontainer($content))); }
}
...
##################################################
##################################################
/*FOLLOWING LOUDBLOG TAGS ADDED BY CHIRAAG*/
##################################################
##################################################
...
function perm_img(){
	...
	if (!empty($_GET['id'])){
		...
		$files = glob('/home/swara/audiowiki/web/loudblog/custom/templates/default/i/ppimages' . '/*.*');
		$c  = count($files);
		$img_string = 'http://cgnetswara.org/loudblog/custom/templates/default/i/ppimages/';
		$imageval = ($imageval % $c);
		$img = $files[$imageval];
		$pos = strrpos($img, '/') + 1;
		$image_name = substr($img, $pos);
		$img_string = $img_string . $image_name;
		return '<img src='. $img_string .' width=220/ >';//"<p>".$img_string."</p>";//
	}

	else {
		
		return "<img src=\"http://cgnetswara.org/loudblog/custom/templates/default/i/training-camp-smaller.jpg\" width=220>";
	}
...
function no_image_link($cur_id){
	$imageval = $cur_id;
# bill - patching to point to swara directory since navneet not restored in 2019
#	$files = glob('/home/navneet/web/loudblog/custom/templates/default/i/ppimages' . '/*.*');
	$files = glob('/home/swara/audiowiki/web/loudblog/custom/templates/default/i/ppimages' . '/*.*');
	$c  = count($files);
# bill - patching to point to swara directory since navneet not restored in 2019
#	$img_string = 'http://cgnetswara.org/navneet/loudblog/custom/templates/default/i/ppimages/';
	$img_string = 'http://cgnetswara.org/loudblog/custom/templates/default/i/ppimages/';
	$imageval = ($imageval % $c);
	$img = $files[$imageval];
	$pos = strrpos($img, '/') + 1;
...
function perm_img_link(){
	global $currentid;
	return no_image_link($currentid);
}
...
// function post_from_dlvrit(){
// 	if (isset($_GET['utm_source']) || isset($_GET['utm_medium'])){
// 		//header('Location: http://cgnetswara.org/index.php?id='+$_GET['id']); 
// 		$id = $_GET['id'];
// 		return "window.location = 'http://cgnetswara.org/index.php?id=$id';" ;
// 	}
// 	return "";
// }
...
function no_of_posts_in_category($content){

	$content = stripcontainer($content);
	$content = fullparse($content);
	$query_to_count = "SELECT COUNT( * ) FROM ".$GLOBALS['prefix']."lb_postings WHERE tags LIKE '%".$content."%'";
	$count = $GLOBALS['lbdata']->GetArray($query_to_count);
	
	$total = $count[0]['COUNT( * )'] ;
		
	return fullparse($total);//fullparse($query_to_count);
...
function loop_postingtags_new($content) {
//returns all tags attached to a posting (works within postings-loop)
global $postings;
global $currentid;
global $currenttag;
...
foreach ($tags as $tag) {
	//ignore empty tags and duplicates
	if($tag == '' || preg_match('/_/i',$tag)) continue;
	if(isset($temparray[$tag])) continue;
	$temparray[$tag] = 1;
	}
	...
	foreach ($tagnames as $tig)  {
	$currenttag = $tig;
	$return .= "<li>".fullparse ($content)."</li>";
}
$lastchars = strlen(strrchr($content,">"))-1;
if ($lastchars > 0) {
    $return = substr($return,0,-$lastchars);
...
function link_permalink_new ($content) {
    global $settings, $currentid;

    if ((!isset($_GET['id'])) OR ($_GET['id'] != $currentid)) {
        $return = "<a href=\"single_new.php?id=" .$currentid. "\"
               title=\"Link to posting\">";
        $return .= fullparse (stripcontainer ($content));
        $return .= "</a>\n";
    } else {
...
function postingtag_new($content) {
//returns a tag belonging to a posting (works within postings-loop)
global $currenttag;
...
$tag = stripusc($currenttag);
if ((isset($att['link'])) AND ($att['link'] == "true")) {
    $return = "<a href=\"index_new.php?tag=".$templinkname."\"
           title=\"All postings with tag ".$tag."\">".$tag."</a>";
} else {
    $return = $tag;
}
...
function contactnumbers(){
	$message = message();
	preg_match_all('/[,:-\s]\p{L}\S*@[0-9]{10,11}/u', $message, $out);
	if (count($out[0])==0){
		return '';
	}
	else {
		$return = "<h4>Contact Numbers:</h4><table>";
		for ( $i=0; $i<count($out[0]); $i++) {
			$splitString = explode('@', $out[0][$i] );
			$return .= "<tr><td>$splitString[0]</td>";
			$return .= "<td>$splitString[1]</td></tr>";		
		}
		$return .= "</table>";
		return $return;
	}
...
function prevpost() {
	global $postings, $currentid, $settings;
	$query = "SELECT id,title,image_file, video_link FROM ".$GLOBALS['prefix']."lb_postings WHERE status = 3 AND posted > '".$postings[$currentid]['posted']."'";
	if (isset($att['impact'])) {
      		$query .= "title LIKE 'Impact%'";
    	}
	$query .= " ORDER BY posted DESC LIMIT 1";
	$query_result = $GLOBALS['lbdata']->GetArray($query);
	if ($query_result)	 {
 		$prev_title = $query_result[0]['title'];
		$prev_id = $query_result[0]['id'];
		$prev_image = (!$query_result[0]['image_file']) ? no_image_link($prev_id) : $settings['url']."audio/".$query_result[0]['image_file'];
        $prev_link = $query_result[0]['video_link'];
        
		return "<a class='prev' href='http://cgnetswara.org/single_new.php?id=".$prev_id."'><span class='icon-wrap'><i class='fa fa-angle-left'></i></span><div><h3>$prev_title</h3><img src=$prev_image alt=''/></div></a>";
	}
	else
		return '';
...
function nextpost() {
	global $postings, $currentid, $settings;
	$query = "SELECT id,title,image_file,video_link FROM ".$GLOBALS['prefix']."lb_postings WHERE status = 3 AND posted < '".$postings[$currentid]['posted']."' ORDER BY posted DESC LIMIT 1";
	$query_result = $GLOBALS['lbdata']->GetArray($query);
	if ($query_result)	 {
 		$next_title = $query_result[0]['title'];
		$next_id = $query_result[0]['id'];
		$next_image = (!$query_result[0]['image_file']) ? no_image_link($next_id) : $settings['url']."audio/".$query_result[0]['image_file'];
        $next_vid_link = $query_result[0]['video_link'];	
		return "<a class='next' href='http://cgnetswara.org/single_new.php?id=".$next_id."'><span class='icon-wrap'><i class='fa fa-angle-right'></i></span><div><h3>$next_title</h3><img src=$next_image alt=''/></div></a>";
...
function link_permalink_text_new ($content) {
    global $settings, $currentid;

    if ((!isset($_GET['id'])) OR ($_GET['id'] != $currentid)) {
      $return = "single_new.php?id=" .$currentid;
    } else {
      $return = "";
    }
    return $return;
...
function permalink_to_post_new ($content) {
    global $settings, $currentid;
    return $settings['url']."/single_new.php?id={$currentid}";
}

#################################################
#################################################

function message_clip() {
    global $postings, $currentid;
    $clipmessage = $postings[$currentid]['message_html'];
    $clipmessage = preg_replace('/<br\s*\/?>/i','',$clipmessage);
    
    $clipmessage = mb_substr($clipmessage, 0 ,150, "UTF-8");
    $clipmessage .= "...<b>Read more</b>";
    return $clipmessage;
...
function link_prev_new($content) {
    $att = getattributes($content);
    $here = (isset($_GET['page'])) ? $_GET['page']: 1;
    ...
    if (($here > 1) AND (!isset($_GET['id']))) {
      if (isset($att['impact'])) {
        $return = '<a href="impact_new.php'.addToUrl('page', ($here - 1)).'" title="Previous Page">';
      } else {
        $return = '<a href="index_new.php'.addToUrl('page', ($here - 1)).'" title="Previous Page">';
      }
        $return .= fullparse(stripcontainer($content));
        $return .= "</a>\n";
    } else {
...
function link_next_new ($content) {
    global $nextpage;
    ...
    if (!isset($_GET['id']) AND ($nextpage == true)) {
      if (isset($att['impact'])) {
        $return  = '<a href="impact_new.php'.addToUrl('page', ($here + 1)).'" title="Next Page">';
      } else {
        $return  = '<a href="index_new.php'.addToUrl('page', ($here + 1)).'" title="Next Page">';
      }
        $return .= fullparse(stripcontainer($content));
        $return .= "</a>\n";
    } else {
...
function author_name($content) {
	...
	if(veryempty($tags)) {$return = "";}
		...
		//ignore empty tags and duplicates
			if($tag == '') continue;
			if (preg_match('/_/i',$tag)){
				$currenttag = stripusc($tag);	
				$return .= "<a href=\"index_new.php?tag=".$tag."\" title=\"All postings with tag ".$currenttag."\">".$currenttag."</a>";
				break;
			}		
